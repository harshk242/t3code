...
const make = Effect.gen(function* () {
  ...
  const configure = Effect.gen(function* () {
    ...
    // On Linux, GNOME matches running windows to .desktop entries via WM_CLASS.
    // Setting `app.setName` to the WM class lets `.deb` installs pin to the dock.
    yield* electronApp.setName(
      environment.platform === "linux" ? environment.linuxWmClass : environment.displayName,
    );
